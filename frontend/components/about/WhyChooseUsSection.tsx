...
function getWhyChooseUsIcon(id: string, isBlue: boolean) {
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {WHY_CHOOSE_US_ITEMS.map((card) => {
            const isBlue = Boolean(card.highlighted);
            return (
              <div
                key={card.id}
              ...
              >
                <div className="space-y-4">
                  ...
                  >
                    {getWhyChooseUsIcon(card.id, isBlue)}
                  </div>
                ...
                <div className="pt-6">
                  <Link
                    href={card.ctaHref}
                    className={`inline-flex items-center justify-center px-6 py-2.5 rounded-full text-sm font-medium transition-all duration-200 shadow-sm focus-visible:outline-2 focus-visible:outline-[#01a7e5] ${
                      isBlue

...
export default function CheckoutForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const generatedId = `RT-${Math.floor(100000 + Math.random() * 900000)}`;
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-3xl p-8 sm:p-12 shadow-md border border-gray-100 text-center my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-slate-50 rounded-2xl p-6 mb-8 text-left space-y-2 border border-gray-100">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-[#01A7E5]">{orderId}</span>
          </div>
          <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-between text-sm pt-2 border-t border-gray-200 font-bold">
            ...
            <span className="text-[#01A7E5]">NPR {grandTotal.toLocaleString()} ({formData.paymentMethod.toUpperCase()})</span>
          </div>
        </div>

        <p className="text-xs text-gray-500 max-w-md mx-auto mb-8 leading-relaxed">
          Our logistics team will contact you at <strong className="text-gray-800">{formData.phone}</strong> or <strong className="text-gray-800">{formData.email}</strong> to verify dispatch details.
        </p>
      ...
      <div className="w-full lg:w-80 shrink-0">
        <div className="bg-white rounded-2xl p-6 shadow-xs border border-gray-100 sticky top-24 space-y-6">
          ...
          <button
            ...
            className="w-full py-4 bg-[#01A7E5] hover:bg-[#018bc0] text-white font-bold rounded-xl shadow-md transition-colors text-center text-base cursor-pointer disabled:opacity-50"
          >
